...
// static
void BraveTooltipPopup::Show(Profile* profile,
                             std::unique_ptr<BraveTooltip> tooltip) {
  DCHECK(profile);
  ...
  g_tooltip_popups[id] = new BraveTooltipPopup(profile, std::move(tooltip));
...
void BraveTooltipPopup::OnCancelButtonPressed(const std::string& tooltip_id) {
  ...
  const BraveTooltip* tooltip = popup->GetTooltip();
  if (!tooltip->attributes().cancel_button_enabled()) {
    return;
  }
...
void BraveTooltipPopup::CreatePopup() {
  ...
  tooltip_view_ = container_view->AddChildView(
      new BraveTooltipView(tooltip_->id(), tooltip_->attributes()));
